...
from mod_dipy.scalespace import ScaleSpace
...
class DiffeomorphicMap(object):
    ...
    def transform(self, image, interpolation='linear', image_world2grid=None,
        ...
        if self.is_inverse:
            ...
            warped[:,:,:,0] = temp_warped
            for idx in range(1,image.shape[-1]):
                warped[:,:,:,idx] = self._warp_backward(image[:,:,:,idx], interpolation,
                                                        image_world2grid, out_shape,
                                                        out_grid2world)
        else:
            ...
            warped[:,:,:,0] = temp_warped
            for idx in range(1,image.shape[-1]):            
                warped[:,:,:,idx] = self._warp_forward(image[:,:,:,idx], interpolation, image_world2grid,
                                                       out_shape, out_grid2world)
        if warped.shape[-1] > 1: ## TPM Registration Normalization
            prob_normalization = np.sum(warped, axis = -1) + 1e-9
            for idx in range(warped.shape[-1]):
                warped[..., idx] = warped[..., idx] / prob_normalization
...
class SymmetricDiffeomorphicRegistration(DiffeomorphicRegistration):
    def __init__(self,
        ...
        self.verbosity = 0 #NONE=0, STATUS=1, DIAGNOSE=2, DEBUG=3
    ...
    def update(self, current_displacement, new_displacement,
        ...
        new_displacement = new_displacement.astype('float32')
        #disp_world2grid = disp_world2grid.astype('float32')
